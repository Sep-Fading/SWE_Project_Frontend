...
const ClaimInfoBox = ({
  ...
  claimAmount,
  date,
  onProcess,  // Add this prop
  onReject    // And this prop
}: ClaimInfoBoxProps & { onProcess: () => void; onReject: () => void; }) => {
  return (
    <div className="flex flex-col shadow-lg rounded-md bg-[#D9D9D9] p-3 md:flex-row md:justify-between">
      ...
      <div className="flex flex-col justify-evenly items-center md:items-start">
        <Button text={buttonText[0]} onClick={onProcess} style="w-[200px]" />
        <Button text={buttonText[1]} onClick={onReject} style="w-[200px]" />
      </div>
    </div>
  );
};
